descriptive.stats.unvacc <- function(out_dir, n_plots, seed_for_plots, get_hh_icc, covs_for_hh_icc) {
  ...
  left join(
    select
      participant_id,
      max(visit_date) as last_visit_date2
    from dat
  ...
  dat_pers <- dat[dat$visit_date==dat$last_visit_date2,]
...
#   
  ...
  # exclude people vaccinated after end of study
  dat_pers_unvacc <- dat_pers[dat_pers$vacc_after_study==0,]

  ### compare covariate distributions between unvaccinated and vaccinated in study
  cov_dist_cont <- cov.dist.cont(
    vars = c("age_at_visit", "time", "time_dose1", "time_dose2"),
    dataset = dat_pers_unvacc,
    exposure = "vaccinated"
  )
  
  write.csv(cov_dist_cont, file=paste(out_dir, "\\cov_dist_cont_vacc_in_study_vs_unvacc.csv", sep=""), row.names=FALSE)
  ...
  write.csv(cov_dist_cat, file=paste(out_dir, "\\cov_dist_cat_vacc_in_study_vs_unvacc.csv", sep=""), row.names=FALSE)
  
  
  
  # exclude people unvaccinated
  dat_pers_vacc_after <- dat_pers[dat_pers$vaccinated==1,]
  
  ### compare covariate distributions between unvaccinated and vaccinated in study
  cov_dist_cont <- cov.dist.cont(
    vars = c("age_at_visit", "time", "time_dose1", "time_dose2"),
    dataset = dat_pers_vacc_after,
    exposure = "vacc_after_study"
  )
  
  write.csv(cov_dist_cont, file=paste(out_dir, "\\cov_dist_cont_vacc_in_study_vs_after.csv", sep=""), row.names=FALSE)
  ...
  write.csv(cov_dist_cat, file=paste(out_dir, "\\cov_dist_cat_vacc_in_study_vs_after.csv", sep=""), row.names=FALSE)
